#!/usr/bin/env bash
# Eval + mechanism for the 12B self-distillation comparison, after training.
#   Model 1: corpus_sft 12B (direct SFT)          -> corpus_sft12b_seed{s}
#   Model 2: on-policy self-distill from Model 1  -> sd12b_seed{s}
# Measures behavioral, forgetting (vs base 12B), probe Δ_newline for all seeds,
# and patching (handoff) for seed 0 of each. Fanned across the 4 GPUs.
set -e
cd ~/on-policy-interp
...
echo "waiting for SD12B training..."
until grep -q SD12B_TRAIN_DONE "$LOG/run_sd12b.log" 2>/dev/null; do sleep 60; done
...
declare -a M=(
  ...
  "sd12b_seed0|runs12b_selfdistill/onpolicy_kd_seed0/ckpt_100"
  "sd12b_seed1|runs12b_selfdistill/onpolicy_kd_seed1/ckpt_100"
  "sd12b_seed2|runs12b_selfdistill/onpolicy_kd_seed2/ckpt_100"
)
...
suite() {  # $1 tag  $2 path  $3 gpu
  ...
  case "$tag" in
    corpus_sft12b_seed0|sd12b_seed0)
